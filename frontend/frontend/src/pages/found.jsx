...
function Found() {
  ...
  const [type, setType] = useState('found'); // Default type to 'found'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await axios.post('http://localhost:3000/found', formData, {
            headers: {
  ...
  return (
    <>
      <Navbar />
      <form onSubmit={handleSubmit} action='/found' method='POST' encType='multipart/form-data' className="found-form">
        ...
        {/* Other Form Fields */}
        <div className="found-date-section">
          <label htmlFor="foundDate">When was the object found?</label>
          <input
            type="date"
            id="foundDate"
            value={foundDate}
